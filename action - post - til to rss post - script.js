...
if (!result) {
    ...
    if (!credential.getValue("jekyll-repo") || !credential.getValue("jekyll-path") || !credential.getValue("working-copy-key")) {
        alert("Repo values are invalid. Please rerun action and enter credentials again.");
        credential.forget();
        ...
        var filePath = credential.getValue("jekyll-path") + '/' + year + '/' + fileName;
        ...
        var baseURL = 'working-copy://x-callback-url/write/?key=' +
                      ...
                      '&text=' + encodeURIComponent(newDraft),
